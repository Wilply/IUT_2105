		...
		<?php include 'admin_panel_menu.php'; ?>
			<div class="commande_detail_list">
				...
				for ($i = 0; $i < mysqli_num_rows($row); $i++) {
				 ...
				 ?>
				 <form class="commande_detail_form" method="POST">
				 	<input type="hidden" name="commande_id"  value=<?php echo '"'.$commande_id.'"'; ?>>
				 	<input type="submit" value="Valider la commande" formaction="valider_commande.php?action=1">
				 	<input type="submit" value="Annuler la commande" formaction="valider_commande.php?action=-1">
				 </form>
			</div>
		</div>
		<div class="footer">
